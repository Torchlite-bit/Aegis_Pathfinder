...
1. **Ask upstream to license their work.** The practical route is a polite
   issue on `brues-code/VanillaGuide-Plus` asking the maintainer to add a
   `LICENSE` file, and noting that the project's own ancestry is unlicensed.
   Many addon authors simply never got round to it and will say yes.
